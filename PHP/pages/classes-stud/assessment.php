<?php
// Include database connection
include 'C:/xampp/htdocs/final_exam-main/PHP/services/config/db_connection.php';

// Retrieve the course name from the URL
$courseName = isset($_GET['course_name']) ? htmlspecialchars($_GET['course_name']) : '';

// Fetch available lessons for the specified course
$query = "SELECT DISTINCT lesson FROM quiz_courses WHERE course_name = ? ORDER BY lesson ASC";
$stmt = $conn->prepare($query);
$stmt->bind_param("s", $courseName);
$stmt->execute();
$result = $stmt->get_result();
$lessons = [];

while ($row = $result->fetch_assoc()) {
    $lessons[] = $row['lesson'];
}
$stmt->close();
?>

<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <script src="https://cdn.tailwindcss.com"></script>
    <title>Choose Quiz for <?php echo $courseName; ?></title>
</head>

<body class="bg-gray-100 font-poppins">

    <!-- Container for centered content -->
    <div class="flex justify-center items-center min-h-screen p-5">
        <div class="bg-white w-full sm:w-3/4 lg:w-1/2 p-8 rounded-lg shadow-lg">

            <!-- Page Heading -->
            <h1 class="text-3xl font-semibold text-center text-indigo-600 mb-6">
                Choose an Assessment for <?php echo htmlspecialchars($courseName); ?>
            </h1>

            <!-- List of Lessons -->
            <div class="space-y-4">
                <?php foreach ($lessons as $lesson): ?>
                    <a href="quiz.php?course_name=<?php echo urlencode($courseName); ?>&lesson=<?php echo urlencode($lesson); ?>"
                        class="block bg-indigo-100 hover:bg-indigo-200 text-indigo-700 p-4 rounded-lg shadow-sm transition duration-200 transform hover:scale-105">
                        Quiz for Lesson <?php echo htmlspecialchars($lesson); ?>
                    </a>
                <?php endforeach; ?>

                <!-- Message if no lessons found -->
                <?php if (empty($lessons)): ?>
                    <p class="text-center text-gray-600">No lessons available for this course.</p>
                <?php endif; ?>
            </div>

        </div>
    </div>

</body>

</html>